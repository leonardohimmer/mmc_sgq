                ...
                <div className={`p-3 border-t border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50 rounded-tr-3xl shrink-0 transition-colors duration-300 ${isCollapsed ? 'flex flex-col items-center gap-3' : ''}`}>
                    ...
                    {!isCollapsed && (
                        <div className="flex justify-center mt-2.5">
                            <span className="text-[9px] text-slate-400 dark:text-slate-500 font-medium tracking-wide">v1.9.4 • Atualizado: {new Date().toLocaleDateString('pt-BR')} {new Date().toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })}</span>
                        </div>
                    )}
                </div>
            </aside>
